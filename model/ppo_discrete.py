...
class PPO_discrete:
    ...
    def choose_action(self, s):
        s = torch.unsqueeze(torch.tensor(s, dtype=torch.float), 0)
        with torch.no_grad():
            dist = Categorical(probs=self.actor(s))  # 创建离散概率分布（Create discrete probability distribution）
            a = dist.sample()   # 从该分布中采样一个动作（Sample an action from this distribution）
            a_logprob = dist.log_prob(a)  # 计算该动作的对数概率 log π(a|s) （Calculate the logarithmic probability of the action）
